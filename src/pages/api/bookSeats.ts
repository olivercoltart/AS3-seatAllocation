...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  for (const category of passengerCategories) {
    ...
    if (category === 'VIP') {
      ...
      filtered = availableSeats.filter((s) => !UNDER10_BLOCKED_ROWS.includes(s.row) && !VIP_ROWS.includes(s.row) && !DISABLED_SEATS.includes(s.id));
    } else if (category === 'Disabled') {
      filtered = availableSeats.filter((s) => DISABLED_SEATS.includes(s.id));
    } else {
      filtered = availableSeats.filter((s) => !VIP_ROWS.includes(s.row) && !DISABLED_SEATS.includes(s.id));
    }
